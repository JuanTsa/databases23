...
def generate_book_entry(i, title):
    ...
    selected_keywords = random.sample(keywords, 3)  # Choose three random keywords from the list
    keyword_string = ', '.join(selected_keywords)  # Join keywords into a comma-separated string
    inventory = generate_random_number(available_copies, 20)
    total_borrowings = generate_random_number(0, 100)

    return f"({i}, {isbn}, {j}, '{title}', '{generate_random_string(publishers)}', {pages}, '{summary}', {available_copies}, '{cover}', '{language}', '{keyword_string}', {inventory}, {total_borrowings}),"
