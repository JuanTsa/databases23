...
def add_book():
    if request.method == "GET":
        ...
        Book_ID=book[0]
        Book_ID=Book_ID+1
        cur.execute("INSERT INTO `Book` (`ISBN`, `School_ID`, `Title`, `Publisher`, `Pages`, `Summary`, `Available_Copies`, `Cover`, `Language`, `Keywords`, `Inventory`)\
                     VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (ISBN, School_ID, Title, Publisher, Pages, Summary, Available_Copies, Cover, Language, Keywords, Inventory))
        db.connection.commit()
